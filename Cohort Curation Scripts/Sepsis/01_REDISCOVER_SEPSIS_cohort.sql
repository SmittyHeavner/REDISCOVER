/*
Filename:
01A_REDISCOVER_Cohort_V4.sql

Purpose:
This script creates a cohort of patients for the REDISCOVER registry.
The patient list is saved in the cohort table, along with other useful data elements.

Description:
This SQL script creates a cohort of hospitalized patients with REDISCOVER
...
-- Drop all tables
DROP TABLE IF EXISTS [Results].[REDISCOVER_Cohort]; -- Change schema as appropriate
DROP TABLE IF EXISTS #REDISCOVER_diagnosis;
DROP TABLE IF EXISTS #inpat;
DROP TABLE IF EXISTS #Vis_Occ;


-- Create cohort table (again specify schema as appropriate)
CREATE TABLE [Results].[REDISCOVER_Cohort] (
...
-- First identify patients (inpatient and outpatient) with REDISCOVER diagnosis
SELECT DISTINCT
    person_id,
    condition_start_date AS diagnosis_date
INTO #REDISCOVER_diagnosis
...
-- Show counts of unique REDISCOVER diagnoses (not unique patients)
SELECT COUNT(*) AS "REDISCOVER_diagnosis_count" FROM #REDISCOVER_diagnosis;

-- Show count of unique patients so far
SELECT COUNT(DISTINCT person_id) AS "REDISCOVER_positive_person_count" FROM #REDISCOVER_diagnosis;

-- REDISCOVER patients with inpatient encounters
SELECT
    v.person_id,
...
INNER JOIN #REDISCOVER_diagnosis AS sd ON v.person_id = sd.person_id
...
SELECT
    COUNT(DISTINCT person_id) AS "REDISCOVER_inpatients",
    COUNT(DISTINCT visit_occurrence_id) AS "REDISCOVER_inpatient_visits"
FROM #inpat;
...
INSERT INTO [Results].[REDISCOVER_Cohort] -- Change schema if not using Results
(
...
-- View data
SELECT TOP 100 *
FROM [Results].[REDISCOVER_Cohort];
