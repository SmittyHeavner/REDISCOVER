...
SELECT
    ...
    100 * COUNT(DISTINCT c.person_id) / (SELECT COUNT(DISTINCT c.person_id) FROM [Results].[deident_REDISCOVER_ID_Person]) AS percent_persons_with_condition
FROM
    [Results].[deident_REDISCOVER_ID_Condition_Occurrence] AS c
INNER JOIN CONCEPT_ANCESTOR
    ON
        ...
        (SELECT condition_concept_id FROM [Results].[deident_REDISCOVER_ID_Condition_Occurrence])
LEFT JOIN CONCEPT
...
SELECT
    [Results].[REDISCOVER_id_concepts].concept_name AS condition_name,
    CONCEPT_ANCESTOR.ancestor_concept_id,
...
INNER JOIN [Results].[REDISCOVER_id_concepts]
    ON CONCEPT_ANCESTOR.ancestor_concept_id = [Results].[REDISCOVER_id_concepts].concept_id
WHERE
    [Results].[REDISCOVER_id_concepts].domain = 'Condition'
    AND (
        [Results].[REDISCOVER_id_concepts].include_descendants = 'TRUE'
        OR CONCEPT_ANCESTOR.ancestor_concept_id = CONCEPT_ANCESTOR.descendant_concept_id
    )
GROUP BY
    CONCEPT_ANCESTOR.ancestor_concept_id, [Results].[REDISCOVER_id_concepts].concept_name
