...
FROM [Results].[deident_REDISCOVER_person] AS p
CROSS JOIN #device_concepts_of_interest AS dci
LEFT JOIN [Results].[deident_REDISCOVER_device_exposure] AS d
    ON
...
SELECT
    ...
    (
        ...
        / (SELECT COUNT(DISTINCT person_id) FROM [Results].[deident_REDISCOVER_person])
    ) AS person_perc
